...
- **marimo, not Jupyter.** Notebooks are plain `.py` files, so they diff and
  ...
  which is how one gets checked without a person watching. JupyterLab is
  installable — it is what the `notebooks` dependency group holds — but what
  is committed here is marimo `.py` files, not `.ipynb`.
- **A plain `uv sync` gives a runnable notebook.** marimo and altair are
  regular dependencies, so nothing extra is needed to open one. The
  `notebooks` group is JupyterLab alone.
...
- **Projected numbers are labelled projected.** R's prediction-loop timing in
  `results/r/RESULTS.md` is 200 customers measured and multiplied by 25, and
  the table it sits in says so. Don't restate such a figure elsewhere — it is
  only meaningful beside the machine and settings that produced it.
...
- **Feature branches** for anything non-trivial, cut from `master`. `origin`
  is `github.com/jleighfields/ergmpy`; push when asked, not by default.
