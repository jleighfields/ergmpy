...
- **Review with the `code-reviewer` agent** before committing: `commit` over
  what is being committed, the full pass before opening a pull request.
...
- Avoid duplication. Two near-identical blocks is a smell; three is a
  refactor. The change-statistic formula is the one to watch: prediction, the
  pseudo-likelihood and the sampler all need it, and it lives in
  `predict.change_statistics` so there is one copy to be wrong.
...
- **`gibbs_updates` is decorated, not duplicated.** `updates_python` and
  `updates_numba` are the same function object, so the readable reference and
  the compiled version cannot drift, and either can serve as the oracle for
  the other. Run with `NUMBA_DISABLE_JIT=1` to execute it as plain Python.
...
- **Feature branches** for anything non-trivial, cut from `main`.
