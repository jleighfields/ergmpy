...
def _(mo):
    ...
    notebook walks the same four parts in Python, explaining what each does.
...
def _(mo):
    ...
    **`edges` cannot be estimated.** The edge count is pinned at 5,000 in
    ...
    coefficient is unidentified. `ergm` reports:
    ...
    **Sampling becomes a per-customer draw.** A move is "customer *i* buys *j* instead of
    *k*", so a Gibbs sweep is one multinomial draw per customer rather than
    a proposal over arbitrary tie toggles.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Part 3 — The statistics, and how they update
...
def _(mo):
    ...
    Moving one customer's purchase from $k$ to $j$ changes only two
    statistics, each in closed form:
    ...
    The R script instead calls `summary(formula)` on the full 5,300-node
    network once per alternative per customer — 25,000 complete
    recomputations — to obtain values that differ by one toggled edge.
    """)
    return
...
def _(train):
    ...
    return mple_fit, mple_seconds, time
...
def _(mo, mple_fit, mple_seconds):
    ...
    `b2star2` comes out at **{mple_fit.coef[7]:.5f}** against a true value near **0.0058** — about
...
def _(mo):
    ...
    reach a `b2star2` of around 537,000 against an observed 299,000, roughly 69
    ...
    estimate to report — but it cannot diverge, and it lands close enough to
    seed MCMLE.

    Excursion length trades the two off. Too short and the draws never leave
    the observed configuration, so the gradient carries no signal; too long and it
...
def _(mo):
    ...
    A comparison against `ergm` means something only if both were asked for
...
def _(mo):
    ...
    `results/r/control_settings.csv`, which is what to check these against.
...
@app.cell
def _(control, mo, mple_fit, run_full_fit, time, train):
    ...
    mo.md(
        f"Contrastive-divergence seed, then MCMLE: **{fit_seconds:.0f} s** "
        f"across {control.n_chains} chains."
        if fit_seconds is not None
        else "Not run. Tick the box to fit here; the coefficients below are "
             "otherwise read from a recorded run."
    )
    return (mcmle_fit,)


@app.cell
def _(ROOT, mcmle_fit, mo, pl):
...
def _(ROOT, load_choice_data, np, python_estimates, time):
    ...
    return accuracy, predict_seconds, test_set
...
def _(accuracy, mo, predict_seconds, test_set):
    ...
    **{predict_seconds * 1000:.1f} ms**.
    ...
    To score one alternative the R script calls `summary(formula)` on the
    whole 5,300-node network, once per alternative per customer, for values
    that differ from each other by a single toggled edge.
    `change_statistics` computes those differences directly, and making the
    same substitution in R would remove the same repeated work.
    """)
    return
...
def _(mo):
    ...
    This implements three term families and one constraint. The direction
    this package takes is not closing that gap term by term — it is the
    constraint axis, where each new constraint reuses the estimation core and only needs
    ...
    and never completes an MCMLE iteration.
